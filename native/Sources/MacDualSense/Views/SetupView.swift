    ...
    var body: some View {
        TimelineView(.periodic(from: .now, by: 1)) { _ in
            VStack(alignment: .leading, spacing: 24) {
                Image(systemName: "gamecontroller.fill").font(.system(size: 44)).foregroundStyle(.blue)
                ...
                GroupBox {
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        Text("Plug in with USB, or pair through System Settings → Bluetooth. For DualSense, hold Create and PS until the light flashes.").font(.callout).foregroundStyle(.secondary)
                        Divider()
                        ...
                        Text("mac-dualsense needs Accessibility access to send the shortcuts you assign. Your configuration and logs stay on this Mac.").font(.callout).foregroundStyle(.secondary)
            ...
            .padding(32).frame(width: 560)
        }
        .onAppear { appState.workspaceSelection.isTestingInput = true }
        .onDisappear { appState.workspaceSelection.isTestingInput = false }
    }
}
